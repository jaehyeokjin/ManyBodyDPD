...
void FitGLE::accumulatePairNormalEquation()
{
    ...
    std::vector<std::vector<double> > deltaf(nall, std::vector<double>(3,0));
    std::vector<std::vector<double> > vparallel(nall, std::vector<double>(3,0));
    std::vector<std::vector<double> > fluctuate(nall, std::vector<double>(3,0));
    ...
    for (int i=0; i<nall; i++)
    {
	    ...
	    double rhoj = trajFrame->density[j];
            ...
            std::vector<double> eij = parallelUnitVector(i, j);
	    ...
	    if (rij < h)
            ...
            vparallel[i][0] += weight * dv[0];
            vparallel[i][1] += weight * dv[1];
            vparallel[i][2] += weight * dv[2];
            vparallel[j][0] -= weight * dv[0];
            vparallel[j][1] -= weight * dv[1];
            vparallel[j][2] -= weight * dv[2];
  
            fluctuate[i][0] += weight * eij[0] * eij[0];
            fluctuate[i][1] += weight * eij[1] * eij[1];
            fluctuate[i][2] += weight * eij[2] * eij[2];
            fluctuate[j][0] += weight * eij[0] * eij[0];
            fluctuate[j][1] += weight * eij[1] * eij[1];
            fluctuate[j][2] += weight * eij[2] * eij[2];
    ...
    for (int i=0; i<nall; i++)
    {
        printf("%d\t%lf\t%lf\t%lf\t%lf\n", i+1, trajFrame->residualForces[i][0], vparallel[i][0], fluctuate[i][0], trajFrame->velocities[i][0]);
    }
