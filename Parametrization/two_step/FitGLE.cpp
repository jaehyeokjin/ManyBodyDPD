...
FitGLE::FitGLE(int argc, char** argv)
{
    ...
    totalSplineCount = info->numSplines;
...
void FitGLE::accumulatePairNormalEquation()
{
    ...
    std::vector<std::vector<double> > pressureForce(nall, std::vector<double>(3, 0));
    ...
    for (int i=0; i<nall; i++)
    {
	    ...
	    if (rij < h)
            ...
            pressureForce[i][0] += -dwdr * (rhoi + rhoj) * dv[0] * 11.009171;
            pressureForce[i][1] += -dwdr * (rhoi + rhoj) * dv[1] * 11.009171;
            pressureForce[i][2] += -dwdr * (rhoi + rhoj) * dv[2] * 11.009171;
            pressureForce[j][0] -= -dwdr * (rhoi + rhoj) * dv[0] * 11.009171;
            pressureForce[j][1] -= -dwdr * (rhoi + rhoj) * dv[1] * 11.009171;
            pressureForce[j][2] -= -dwdr * (rhoi + rhoj) * dv[2] * 11.009171;
    ...
    for (int m=0; m<nSplines; m++)
    {
        ...
        for (int k=0; k<3 * nall; k++)
        {
            sum_b += frameMatrix[m][k] * (trajFrame->residualForces[k/3][k%3] - pressureForce[k/3][k%3]);
        }
...
void FitGLE::leastSquareSolver(bool isPair)
{
    ...
    if (isPair)
    {
        for (int m=0; m<info->numSplines; m++)
        {
            ...
            //splineCoefficientsDensity[m] = b[m + info->numSplines];
        }
    }
    else
    {
